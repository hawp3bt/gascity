...
func TestOrphanSweepPreservesProtectedInProgressEphemeralMoleculeWisp(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, requiredCall := range []string{
				...
				if countExactLine(lines, requiredCall) == 0 {
					t.Fatalf("missing required gc call %q\n%s", requiredCall, orphanSweepFailureContext(out, gcLog))
				}
			}
		})
	}
...
func linkOrphanSweepCleanroomTools(t *testing.T, binDir string) {
	t.Helper()
...
if [ "$*" = "--rig project-alpha session list --json" ]; then
...
func orphanSweepProtectedWispBeadsJSON(t *testing.T, protectedID, protectedAssignee, orphanID, orphanAssignee string) string {
	t.Helper()
	// orphan-sweep only consumes the assignee rows here; the extra fields keep
	// the fixture shaped like the ephemeral molecule wisps from bug #1578.
	type bead struct {
		ID        string `json:"id"`
